...
def rnn_cell_backward(da_next, cache):
    ...
    dz = np.multiply((1 - np.square(a_next)),da_next)
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    da_prev = np.matmul(parameters['Wf'][:, :n_a].T, dft) + np.matmul(parameters['Wi'][:, :n_a].T, dit) + np.matmul(
        parameters['Wc'][:, :n_a].T, dcct) + np.matmul(parameters['Wo'][:, :n_a].T, dot)
    dc_prev = dc_next * ft + ot * (1 - np.square(np.tanh(c_next))) * ft * da_next
    dxt = np.matmul(parameters['Wf'][:, n_a:].T, dft) + np.matmul(parameters['Wi'][:, n_a:].T, dit) + np.matmul(
        parameters['Wc'][:, n_a:].T, dcct) + np.matmul(parameters['Wo'][:, n_a:].T, dot)
    ### END CODE HERE ###
