...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        out2 = self.layernorm2(ffn_output + out1)  # (batch_size, input_seq_len, fully_connected_dim)
        # END CODE HERE
...
def Encoder_test(target):
    ...
    x.shape[0], x.shape[1], embedding_dim), f"Wrong shape. We expected ({eshape[0]}, {eshape[1]}, {embedding_dim})"
...
def DecoderLayer_test(target):
    ...
    assert np.allclose(attn_w_b1[0, 0, 0], [0, 0.5, 0.5],
                       atol=1e-2), "Wrong values in attn_w_b1. Check the call to self.mha1"
    assert np.allclose(attn_w_b2[0, 0, 1],
                       [0.34485385, 0.3323007, 0.32284543]), "Wrong values in attn_w_b2. Check the call to self.mha2"
    ...
    assert np.allclose(out[0, 0], [0.59296525, -1.4068702, 1.224841,
                                   -0.410936]), "Wrong values in out when we mask the last word. Are you passing the padding_mask to the inner functions?"
...
def Transformer_test(target):
    ...
    assert np.allclose(weights[keys[0]][0, 0, 1],
                       [0., 0., 0.31332517, 0.33401096, 0.35266393]), f"Wrong values in weights[{keys[0]}]"
